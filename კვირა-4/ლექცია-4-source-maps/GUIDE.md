...
### 2.1 Dev რეჟიმი vs Production Build

სანამ build-ს გავუშვებთ, გავარკვიოთ რა განსხვავებაა `npm run dev`-სა და `npm run build`-ს შორის — ეს ორი სრულიად სხვადასხვა რეჟიმია. პირველი **დეველოპერისთვის** არის ოპტიმიზებული, მეორე — **საბოლოო მომხმარებლისთვის**.

#### `npm run dev` — dev სერვერი (ბილდი ფაქტობრივად არ ხდება)

Vite dev რეჟიმში bundle-ს **არ ქმნის**. ის უშვებს სერვერს, რომელიც თითოეულ ფაილს ცალკე გასცემს native ES module-ად, ბრაუზერის მოთხოვნისთანავე:

- `App.jsx` ბრაუზერამდე თითქმის ისე მიდის, როგორც დაწერეთ — მხოლოდ JSX გარდაიქმნება JS-ად
- მინიფიკაცია **არ ხდება** — ცვლადების სახელები, კომენტარები და ხაზები ხელუხლებელია
- Source maps **ავტომატურად ჩართულია** — ცალკე კონფიგურაცია არ სჭირდება
- **HMR** — ფაილის შენახვისთანავე მხოლოდ შეცვლილი მოდული ახლდება, გვერდის reload-ის გარეშე
- React მუშაობს **development build**-ით — სრული warning-ები Console-ში (`key` prop, `setState` render-ში და ა.შ.)
- `dist/` ფოლდერი არ იქმნება — ყველაფერი მეხსიერებაშია

#### `npm run build` — production ბილდი

აქ Vite (Rollup + Terser) მთელ პროექტს ერთხელ ამუშავებს და შედეგს `dist/`-ში წერს:

- **Bundling** — ყველა მოდული ერთიანდება რამდენიმე ფაილში
- **Minification** — `terser` ჭრის სივრცეებს და კომენტარებს, ცვლადებს არქმევს `a`, `b`, `c`
- **Tree-shaking** — გამოუყენებელი კოდი ამოიშლება
- **Hash-იანი სახელები** — `dist/assets/index-C_rNfLOx.js` (cache busting)
- React გადადის **production build**-ზე — warning-ები და dev-only შემოწმებები ამოჭრილია
- Source maps **გამორთულია**, სანამ `vite.config.js`-ში არ ჩართავთ (ამ პროექტში ახლა `sourcemap: false`)

**რიცხვებში ამ პროექტში:** `src/` ≈ 39 KB წაკითხვადი კოდი → `dist/assets/index-*.js` ≈ 164 KB (React-ის ჩათვლით), ერთ ხაზზე, სრულიად წაუკითხავი.

#### შედარების ცხრილი

| | `npm run dev` | `npm run build` + `npm run preview` |
|---|---|---|
| Bundling | არ ხდება (ცალკეული ES modules) | ერთიანი, tree-shaken |
| Minification | არა | კი (terser) |
| Source maps | ავტომატურად კი | მხოლოდ თუ ჩართავთ |
| React warnings | სრული | ამოჭრილი |
| Stack trace | `validateEmail (validation.js:15:11)` | `a (index-abc123.js:1:4567)` |
| Rebuild | HMR, მყისიერი | სრული build, წამები |
| გამოყენება | დეველოპმენტი | deploy |

> **მნიშვნელოვანი ნიუანსი: „ლოკალური ბილდი" ≠ „dev რეჟიმი".** თუ ლოკალურად გაუშვებთ `npm run build && npm run preview`, მიიღებთ **ზუსტად იმავე** არტეფაქტს, რაც სერვერზე წავა — იგივე მინიფიკაცია, იგივე hash-ები. სწორედ ამიტომ არის ეს ეტაპი `preview`-ზე აგებული: ლოკალურად ვამრავლებთ production-ის პრობლემას, რომ დავინახოთ რას წყვეტს source maps.

ლოკალურ და რეალურ production ბილდს შორის განსხვავება ჩვეულებრივ მხოლოდ ესაა:

- **გარემოს ცვლადები** — `.env.production` vs `.env.local` (API მისამართები, key-ები)
- **CI/CD ნაბიჯები** — Sentry-ზე `.map` ფაილების ატვირთვა, CDN-ზე deploy
- **`base` path** — თუ საიტი ქვე-დირექტორიაშია
